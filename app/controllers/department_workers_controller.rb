...
class DepartmentWorkersController < ApplicationController
  ...
  def new
    ...
    if params[:worker_id].present?
      ...
      if @worker.worker_positions.blank? || @worker.worker_positions.last.end_date.present?
        redirect_to new_worker_position_url(worker_id: @worker.id),
                    alert: t('alert.create.department_worker_position')
      end
    end
  ...
  def edit
    ...
    return unless @manager.present? && @department_worker.worker_id != @manager.worker_id

    redirect_to department_url(@department_worker.department),
                alert: t('alert.new.department_worker_manager_present')
  end
  ...
  def create
    ...
    if @manager.present? && @department_worker.status == 'manager'
      redirect_to new_department_worker_path(department_worker_params),
                  alert: t('alert.new.department_worker_manager_present')
    elsif @department_worker_count <= 19
      ...
      redirect_to new_department_worker_path(department_worker_params),
                  alert: t('alert.new.department_worker')
    end
  end
  ...
  def destroy
    ...
    if active_vacation.present? && active_vacation.start_date <= this_day && active_vacation.end_date >= this_day
      active_vacation.update(duration_days: (this_day - active_vacation.start_date).to_i, end_date: this_day)
    end
    active_vacation.destroy if active_vacation.present? && active_vacation.start_date > this_day
  ...
  def my_formhelper
    ...
    @free_workers = Worker.unscoped.where(date_of_fired: [nil, ''], id: worker_active_position)
                          .where.not(id: busy_workers).order(:roll_number).pluck(:roll_number, :id)
  end
