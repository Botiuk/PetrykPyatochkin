...
class DepartmentsController < ApplicationController
  ...
  def index
    @pagy, @departments = pagy(Department.order(:name), items: 20)
    @department_workers_count = DepartmentWorker.group(:department_id).count
  ...
  def show
    ...
    @department_manager = Worker.joins(:department_worker).where(department_worker: { department_id: @department.id,
                                                                                      status: 'manager' }).first
    @active_vacations = Vacation.where(worker_id: department_workers_ids).where.not(end_date: ...Time.zone.today)
    @worker_id_active_positions = WorkerPosition.where(worker_id: department_workers_ids,
                                                       end_date: nil).pluck(:worker_id)
  end
