...
RSpec.describe WorkerPosition do
  describe 'validations' do
    it 'is valid with valid attributes' do
      worker_position = build(:worker_position, end_date: Time.zone.today)
    ...
    it 'is not valid without a worker' do
      ...
      expect(worker_position).not_to be_valid
    end
    ...
    it 'is not valid when start_date is greater than end_date' do
      worker_position = build(:worker_position, start_date: Time.zone.today, end_date: (Time.zone.today - 1))
      expect(worker_position).not_to be_valid
    end

    it 'is valid when start_date is equal end_date' do
      worker_position = build(:worker_position, start_date: Time.zone.today, end_date: Time.zone.today)
      expect(worker_position).to be_valid
    end
  end
